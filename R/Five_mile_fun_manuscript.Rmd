...
my_ggsave("M15131_figure_2.tif", spp_over_time_plot,
          height = 81,
          width = 105)
my_ggsave("M15131_figure_5.tif", diversity_and_abundance_plot,
          height = 81,
          width = 105)
my_ggsave("M15131_figure_3.tif", three_trait_space_plot,
          height = 190,
          width = 140)
# my_ggsave("ordination.pdf", all_comms_plot, # pdf to allow editing in adobe
#           height = 2400,
...
#           device = 'pdf',
#           units = 'px')
my_ggsave("M15131_figureS2.tif", cormp_plot, # pdf to allow editing in adobe
          height = 190,
          width = 140)
```
...
italics_file <- "./Data/spp_italics.csv"
...
italics_key <- read_csv(italics_file)
...
trait_plot <- trait_pcoa$vectors %>% 
    ...
    left_join(italics_key, join_by("rowname" == "Species")) %>%
    mutate(Name_pretty = if_else(Italics,
                                 paste("italic('",
                                       rowname,
                                       "')", sep = ""),
                                 str_replace_all(rowname, " ", "~"))) %>%
    select(-Italics)
...
mfa_cwm_plot <- ggplot() +
    ...
    ggrepel::geom_label_repel(data = mfa_contrib,
                              aes(x = Dim.1 * 2 + 0.5, 
                                  y = Dim.2 * 2 + 0.5, 
                                  label = Trait_value_pretty, 
                                  fill = Trait),
                              show.legend = T, 
                              size = small_text_size/.pt,
                              seed = 3, 
...
cwm_plot <- ggarrange(cwm_1_density, NA, mfa_cwm_plot, cwm_2_density,
          ...
          align = 'hv',
          common.legend = T) %>%
    annotate_figure(left = text_grob(paste('MFA Axis 2: ', round(dim2_mfa, 2), '%'), 
                                     size = lab_text_size, rot = 90), 
                    bottom = text_grob(paste('MFA Axis 1:', round(dim1_mfa, 2), '%'), 
                                       size = lab_text_size))
...
pcoa_comm_plot <- ggplot() +
    ...
    ggrepel::geom_label_repel(data = pcoa_contrib,
                              aes(x = Axis.1 * 2, 
                                  y = Axis.2 * 2, 
                                  label = Name_pretty, 
                                  fill = Species %in% colnames(falgae)),
                              show.legend = T, 
                              size = small_text_size/.pt,
                              seed = 3, 
                              min.segment.length = 10,
...
diversity_and_abundance_plot <- ggarrange(abundance_plot,
                                          diversity_plot,
                                          ncol = 1,labels = c('A', 'B'))
```
